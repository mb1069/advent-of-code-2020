...
class GameOfLife4D:
    def __init__(self, initial_state):
        ...
        self.z = 1
        self.w = 1
        self.cube = np.zeros((1, 1, self.y, self.x))
    ...
    def expand_borders(self):
        ...
        w_pad = self.get_w_pad()

        npad = (w_pad, z_pad, y_pad, x_pad)
        ...
        print('\t', new_shape)
        self.w, self.z, self.y, self.x = new_shape
    ...
    def get_sum_adjacent_cells(self, x, y, z, w):
        min_w = max(w - 1, 0)
        max_w = w + 2
        ...
        subcube = self.cube[min_w:max_w, min_z:max_z, min_y:max_y, min_x:max_x]
        total = subcube.sum() - self.cube[w, z, y, x]
        return total

    def gen_next_cube(self):
        self.expand_borders()
        newcube = np.zeros((self.w, self.z, self.y, self.x))
        ws = list(range(self.w))
        zs = list(range(self.z))
        ys = list(range(self.y))
        xs = list(range(self.x))
        for w, z, y, x in product(ws, zs, ys, xs):
            cube_active = self.cube[w, z, y, x] == 1
            active_neighbours = self.get_sum_adjacent_cells(x, y, z, w)
            if cube_active:
